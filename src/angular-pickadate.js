(function(angular) {
    ...
    angular.module('pickadate', []).provider('pickadateI18n', function() {
        var defaults = {
            'prev': 'prev',
            'next': 'next'
        };
        ...
        this.$get = function() {
            ...
            return {
                t: function(key) {
                    return translations[key] || defaults[
                        key];
                }
            };
        };
    }).factory('pickadateModalBindings', ['$window', '$document',
        function($window, $document) {
            var supportPageOffset = $window.pageXOffset !==
                undefined,
                isCSS1Compat = (($document.compatMode || "") ===
                    "CSS1Compat");
            var computeStyles = function(element) {
                var scrollX = supportPageOffset ? $window.pageXOffset :
                    ...
                    styles = {
                        top: scrollY + element.getBoundingClientRect()
                            .bottom + 'px'
                    };
                if ((innerWidth - element.getBoundingClientRect()
                    .left) >= 300) {
                    styles.left = scrollX + element.getBoundingClientRect()
                        .left + 'px';
                } else {
                    styles.right = innerWidth - element.getBoundingClientRect()
                        .right - scrollX + 'px';
                }
                return styles;
            };
            ...
            return function(scope, element, rootNode) {
                ...
                element.on('focus', function() {
                    scope.modalStyles = computeStyles(
                        element[0]);
                    togglePicker(true);
                });
                element.on('keydown', function(e) {
                    if (indexOf.call([9, 13, 27], e.keyCode) >=
                        0) togglePicker(false);
                });
                $document.on('click', function(e) {
                    if (isDescendant(rootNode, e.target) ||
                        e.target === element[0]) return
                    ;
                    togglePicker(false);
                });
            };
        ...
        function($locale, dateFilter) {
            ...
            return function(format, options) {
                var minDate, maxDate, disabledDates,
                    currentDate, weekStartsOn, noExtraRows;
                options = options || {};
                format = format || 'yyyy-MM-dd';
                weekStartsOn = options.weekStartsOn;
                noExtraRows = options.noExtraRows;
                if (!angular.isNumber(weekStartsOn) ||
                    weekStartsOn < 0 || weekStartsOn > 6)
                    weekStartsOn = 0;
                return {
                    parseDate: function(dateString) {
                        ...
                        if (isNaN(dateObj.year) || isNaN(
                            dateObj.month) || isNaN(
                            dateObj.day)) return;
                        return new Date(dateObj.year,
                            dateObj.month - 1, dateObj.day,
                            3);
                    },
                    setRestrictions: function(restrictions) {
                        minDate = this.parseDate(
                                restrictions.minDate) ||
                            new Date(0);
                        maxDate = this.parseDate(
                    ...
                    buildDateObject: function(date) {
                        var localDate = angular.copy(date),
                            formattedDate = dateFilter(
                                localDate, format),
                            disabled = indexOf.call(
                                disabledDates,
                                formattedDate) >= 0,
                            monthOffset = this.getMonthOffset(
                                localDate, currentDate),
                            outOfMinRange = localDate <
                            minDate,
                            outOfMaxRange = localDate >
                            maxDate,
                            outOfMonth = (monthOffset === -
                                1 && !options.previousMonthSelectable
                            ) || (monthOffset === 1 && !
                                options.nextMonthSelectable
                            );
                        return {
                            date: localDate,
                    ...
                    buildDates: function(year, month, options) {
                        var dates = [],
                            date = new Date(year, month, 1,
                                3),
                            lastDate = new Date(year, month +
                                1, 0, 3);
                        options = options || {};
                        currentDate = angular.copy(date);
                        while (date.getDay() !==
                            weekStartsOn) date.setDate(date
                            .getDate() - 1);
                        for (var i = 0; i < 42; i++) { // 42 == 6 rows of dates
                            if (noExtraRows && date.getDay() ===
                                weekStartsOn && date >
                                lastDate) break;
                            dates.push(this.buildDateObject(
                                date));
                    ...
                    buildDayNames: function() {
                        var dayNames = $locale.DATETIME_FORMATS
                            .SHORTDAY;
                        if (weekStartsOn) {
                            dayNames = dayNames.slice(0);
                            for (var i = 0; i <
                                weekStartsOn; i++) dayNames
                                .push(dayNames.shift());
                        }
                        return dayNames;
                    },
                    getMonthOffset: function(date1, date2) {
                        return date1.getMonth() - date2.getMonth() +
                            (12 * (date1.getFullYear() -
                                date2.getFullYear()));
                    }
                };
            };
        }
    ]).directive('pickadate', ['$locale', '$sce', '$compile',
        '$document', '$window', 'pickadateDateHelper',
        'pickadateI18n', 'pickadateModalBindings', 'filterFilter',
        function($locale, $sce, $compile, $document, $window,
            dateHelperFactory, i18n, modalBindings, filter) {
            var TEMPLATE =
                '<div class="pickadate" ng-show="displayPicker" ng-style="modalStyles">' +
                    '<div class="pickadate-title">' +
                        '<a href="" class="pickadate-prev pickadate-control" ng-click="changeMonth(-1)" ng-show="allowPrevMonth">' +
                            $sce.trustAsHtml(i18n.t('prev')) + 
                        '</a>' +
                        '<span class="pickadate-header">' +
                            '{{currentDate | date:"MMMM yyyy"}}' + 
                        '</span>' +
                        '<a href="" class="pickadate-next pickadate-control" ng-click="changeMonth(1)" ng-show="allowNextMonth">' +
            ...
            return {
                ...
                link: function(scope, element, attrs, ngModel) {
                    var allowMultiple = attrs.hasOwnProperty(
                            'multiple'),
                        selectedDates = [],
                        ...
                        dateHelper = dateHelperFactory(format, {
                            previousMonthSelectable: /^(previous|both)$/
                                .test(attrs.selectOtherMonths),
                            nextMonthSelectable: /^(next|both)$/
                                .test(attrs.selectOtherMonths),
                            weekStartsOn: scope.weekStartsOn,
                            noExtraRows: attrs.hasOwnProperty(
                                'noExtraRows')
                        });
                    scope.displayPicker = !wantsModal;
                    scope.setDate = function(dateObj) {
                        if (!dateObj.enabled) return;
                        selectedDates = allowMultiple ?
                            toggleDate(dateObj,
                    ...
                    var $render = ngModel.$render = function(
                        ...
                        selectedDates = map(selectedDates,
                            function(date) {
                                return dateHelper.buildDateObject(
                    ...
                    scope.classesFor = function(date) {
                        var formattedDates = map(
                                selectedDates,
                                'formattedDate'),
                            classes = indexOf.call(
                                formattedDates, date.formattedDate
                            ) >= 0 ? 'pickadate-active' :
                            null;
                        return date.classNames.concat(
                            classes);
                    };
                    ...
                    if (wantsModal) {
                        ...
                        element.after(compiledHtml.addClass(
                            'pickadate-modal'));
                    } else {
                    ...
                    function render() {
                        var dates = dateHelper.buildDates(
                        ...
                        scope.dates = map(dates, function(
                            date) {
                    ...
                    function setViewValue(value, options) {
                        options = options || {};
                        if (allowMultiple) {
                            ngModel.$setViewValue(map(value,
                                'formattedDate'));
                        } else {
                    ...
                    function toggleDate(dateObj, dateArray) {
                        var index = indexOf.call(dateArray,
                            dateObj);
                        if (index === -1) {
